...
def clean_dataframe(df):
    ...
    numeric_cols = [
        "Surcharge (SLL)","Platform Fee (SLL)","Delivery Duration (mins)",
        "Max Price (SLL)","Rating","Shopper Earning (SLL)"
    ]
...
if "requests" not in st.session_state:
    st.session_state.requests = pd.DataFrame(columns=[
        ...
        "Platform Fee (SLL)","Shopper Earning (SLL)","Payment Type",
        "Payment Settled"
    ])
...
shopper_bases = {
    "Lumley": (8.4571, -13.2924),"Aberdeen": (8.4848, -13.2827),"Congo Cross": (8.4842, -13.2673),
    "Campbell Street": (8.4865, -13.2409),"Calaba Town": (8.3786, -13.1664),"Jui": (8.3543, -13.1216),
    "Siaka Stevens Street": (8.4867, -13.2349),"Circular Road": (8.4830, -13.2260),"Eastern Police": (8.4722, -13.2167),
    "Rawdon Street": (8.4856, -13.2338),"New England": (8.4746, -13.2500),"Hill Station": (8.4698, -13.2661),
...
if user_type == "Admin":
    ...
    # Unsettled Mobile Money
    unsettled_mm = df[(df["Payment Type"]==t("Mobile Money","Mobile moni")) & (df["Payment Settled"]=="No")]
    st.subheader(t("Unsettled Mobile Money Payments","Mobile Money we no don settle"))
    st.write(unsettled_mm[["Tracking ID","Requester","Surcharge (SLL)","Shopper Name","Preferred Shopper Base"]])
    st.download_button(
        label=t("Download Unsettled Mobile Money Report","Download unsettled MM ripot"),
        data=unsettled_mm.to_csv(index=False),
        file_name="unsettled_mobile_money.csv"
    )
    st.stop()
...
if user_type == t("Requester","Pesin we dae oda"):
    ...
    shopper_earning = int(selected_surcharge*0.90)  # 10% platform fee

    if st.button(t("Submit Request","Send oda")):
        tracking_id = str(uuid.uuid4())[:8]
        platform_fee = int(selected_surcharge*0.10)
        new_row = {
            "Tracking ID":tracking_id,"Requester":name,"Requester Contact":contact,"Campus":campus,
            "Item":item,"Qty":qty,"Max Price (SLL)":max_price,"Expected Delivery Time":delivery_time.strftime("%H:%M"),
            "Preferred Shopper Base":preferred_base,"Surcharge (SLL)":selected_surcharge,
            "Assigned Shopper":"Unassigned","Shopper Name":"","Timestamp":datetime.utcnow().isoformat(),
            "Status":"Pending","Rating":"","Accepted Time":"","Delivered Time":"",
            "Delivery Duration (mins)":"","Platform Fee (SLL)":platform_fee,"Shopper Earning (SLL)":shopper_earning,
            "Payment Type":payment_type,"Payment Settled":"No"
        }
    ...
    if available_df.empty:
        ...
        if st.button(t("Accept Request","Tek dis oda")):
            if track_id_input in available_df["Tracking ID"].values:
                idx = st.session_state.requests.index[st.session_state.requests["Tracking ID"]==track_id_input][0]
    ...
    if not my_jobs.empty:
        ...
        if st.button(t("Update Status","Update Status")):
            if update_id in my_jobs["Tracking ID"].values:
                idx = st.session_state.requests.index[st.session_state.requests["Tracking ID"]==update_id][0]
        ...
        # Mobile Money settlement confirmation
        for idx, row in my_jobs.iterrows():
            if row["Payment Type"]==t("Mobile Money","Mobile moni") and row["Payment Settled"]=="No":
                if st.checkbox(f"{t('Mark Mobile Money as received','Tek Mobile Money')}: {row['Tracking ID']}", key=f"mm_{idx}"):
                    st.session_state.requests.at[idx,"Payment Settled"]="Yes"
                    save_requests_to_gsheet()
                    st.success(t("Payment marked as received!","Moni don mark!"))
