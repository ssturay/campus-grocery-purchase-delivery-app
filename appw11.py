...
if "requests" not in st.session_state:
    st.session_state.requests = pd.DataFrame(columns=[
        ...
        "Preferred Shopper Base","Distance (km)","Surcharge (SLL)",
...
if user_type == "Admin":
    ...
    col5.metric(t("Total Surcharge", "Ol Surcharge"), total_surcharge)
    col6.metric(t("Platform Fee", "Platform Fee"), total_platform_fee)

    delivered_df = df[df["Status"]=="Delivered"]
    if not delivered_df.empty:
        avg_time = delivered_df["Delivery Duration (mins)"].astype(float).mean()
        st.metric(t("Avg Delivery Time (mins)","Avrej ten fo dilivri"), round(avg_time,1))

    st.subheader(t("Revenue by Campus","Moni we kampus mek"))
    rev_by_campus = df.groupby("Campus")["Surcharge (SLL)"].sum().astype(float)
    st.bar_chart(rev_by_campus)
...
if user_type == t("Requester","Pesin we dae oda"):
    ...
    for base_name,(base_lat,base_lon) in shopper_bases.items():
        ...
        distance_options[base_name] = round(dist,2)
        surcharge_options[base_name] = calculate_surcharge(dist)

    surcharge_df = pd.DataFrame([
        {
            "Shopper Base": k,
            t("Distance (km)","Distans (km)"): distance_options[k],
            t("Estimated Surcharge (SLL)","Moni fo dilivri"): surcharge_options[k]
        }
        for k in sorted(surcharge_options, key=surcharge_options.get)
    ])
    ...
    selected_distance = distance_options[preferred_base]

    # Map with route
    m = folium.Map(location=[lat, lon], zoom_start=14)
    folium.Marker([lat, lon], tooltip=t("Campus","Kampus")).add_to(m)

    base_lat, base_lon = shopper_bases[preferred_base]
    folium.Marker([base_lat, base_lon], tooltip=preferred_base, icon=folium.Icon(color='green')).add_to(m)

    folium.PolyLine(
        locations=[(lat, lon), (base_lat, base_lon)],
        color="blue",
        weight=3,
        tooltip=t("Delivery Route","Rout fo dilivri")
    ).add_to(m)
    ...
    if st.button(t("Submit Request","Send oda")):
        ...
        if not name or not contact or not item:
            st.warning(t("Please fill all required fields.","Plis ful-ful ol box dem."))
            st.stop()
